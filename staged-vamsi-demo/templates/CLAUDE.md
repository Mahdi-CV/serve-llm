...
`<CONTAINER>` (after the image `<IMAGE>`). It runs an idle
...
- When the user asks for "<MODEL_SPOKEN>", serve the variant `<MODEL>`.
  That is our standard precision/variant for this model on this box. Do not ask which
  precision to use.
...
2. If the container `<CONTAINER>` is running but the model is not being
   served, start it inside the container and wait for `/health` to return 200:

       docker exec -d <CONTAINER> bash -lc \
         "vllm serve <MODEL> --trust-remote-code <VLLM_MODEL_ARGS> \
          --port <PORT> > /tmp/vllm.log 2>&1"

   then poll `curl -sf http://localhost:<PORT>/health` until it succeeds.
...
- Add any model-specific client note here (e.g. reasoning/thinking toggles, chat
  template kwargs) so the test request returns a direct answer.
